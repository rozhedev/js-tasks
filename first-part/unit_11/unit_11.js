...
function goProgram(btn, func) {
    document.querySelector(btn).onclick = func;
}
...
/* Теперь я понял почему хороший програмист - это ленивый програмист)
Думаю правильней сказать так:
Хороший програмист - это програмист, который уменшает время на выполнение определенной задачи,
путем ОПТИМИЗАЦИИ РУТИННЫХ ДЕЙСТВИЙ. Правильно?
Я беру пример с вас и делаю также) функциями выше. Думаю, все понятно.
*/
...
function f6() {
    d6[d6.length - 1] += ' ' + getInput('.i-6').value + ' ';
...
function f8() {
    // d8[0] += getInput('.i-8').value + ' ';    или так
    d8.splice(0, 1, getInput('.i-8').value);
...
function f11() {
    let inp = +getInput('.i-11').value;
    let searchedItem = d11.indexOf(inp);

    document.querySelector('.out-11').textContent = searchedItem;
}
...
function f13() {
    ...
    // showArr('.out-13', newArr);      Функция выводит с пробелами.
...
function f15() {
    let inp = +getInput('.i-15').value;
    let searchedItem = d15.indexOf(inp);
...
function f16() {
    let concatArr = d161.concat(d162);

    showArr('.out-16', concatArr);
}
...
function f18() {
    if (d18.includes(getInput('.i-18').value)) {

        document.querySelector('.out-18').textContent = true;
    }
    else{
        document.querySelector('.out-18').textContent = false;
    }
}
...
// Task 20
// Напишите функцию f20, которая применяет к массиву d20 метод join и соединяет элементы массива в одну строку. Результат выводит в .out-20.
// Во всех задачах где говорим второй, третий и т.д. имеем ввиду индекс
...
function f20() {
    d20.join();
    document.querySelector('.out-20').textContent = d20;
}
