...
console.log('Цветом он отличается!');
...
document.querySelector('.three').innerHTML = '<h3>Новосозданный заголовок</h3>';
...
document.querySelector('.four').innerHTML += '<h4>New header</h4><p>new paragraph</p>';
...
a.innerHTML = `${Math.PI} дробные числа пишем через точку!`;      // так можна сделать?
// a.innerHTML = 3.1415;    // 2 способ
...
// С ваших уроков я научился задавать константы если значение переменной не будет переписыватся.
// В даном случае мы не работаем с атрибутами (как например смена value в input), а просто записываем
// содержимое внутрь тэга.
// Правильно ли я делаю? Для меня важен ваш ответ) вы круто объясняете.

let y1 = 6;
let y2 = 3;

secondResult.innerHTML = y1 / y2;
...
thirdResult.innerHTML = x1 * x2 + ' /загляните в консоль';
console.log('Task 16. Вывело NaN (not a number) я понял что это особый тип даных, (как null или underfined) он выводится если призводить математические действия с числом или сторокой (но если число и строку сложить произойдет конкатенация /склеивания в одну строку/)');
...
// Мы получили весь елемент(объект) со всеми его свойствами (правая боковая панель > Properties)
...
d2 = 5;
console.log(d2);
// Мы присвоили переменной новое значение, а старое просто исчезло.
...
// Аналогично к предыдущему заданию
...
// let emptyBody = document.querySelector('body');
// emptyBody.innerHTML = '';

// Все DOM - дерево исчезло так как присвоили пустую строку!